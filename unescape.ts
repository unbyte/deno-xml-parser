...
function codePointToSymbol(codePoint: number, strict: boolean): string {
  let output = ''
...
export function unescapeEntity(
  raw: string,
  options?: Partial<UnescapeOptions>,
) {
  const { strict, isAttributeValue } = {
    strict: false,
    isAttributeValue: false,
    ...options,
  }

  if (strict && regexInvalidEntity.test(raw)) {
    parseError('malformed character reference')
  }

  return raw.replace(regexDecode, function($0, $1, $2, $3, $4, $5, $6, $7, $8) {
    let codePoint
    let semicolon
    let decDigits
    let hexDigits
...
const invalidReferenceCodePoints = new Set([
  1,
  2,
  3,
  4,
...
const decodeMapNumeric: Record<number, string> = {
  0: '\uFFFD',
  128: '\u20AC',
  130: '\u201A',
  131: '\u0192',
